...
\textbf{TODO}
\begin{itemize}
    \item (Russell) Find out what are already known/considered for commit transactions (or however it is called)
    \item (Russell) Discuss "natural applications" of commit transactions, beyond atomic swaps, e.g. P2P escrow system 
    \item Rename ``CommmtTx for public ledger'' to ``CommitTx for non-UTXO (or however it is called)''. Write down CommitTx interface for UTXO
