...
The protocol is asymmetric, with one party acting as an exchange server, force opening a refund timed commitment with timelock $T_0$ , and the other as a client who is required to compute, at worst, a timed commitment equivalent to $2\mathbb{B}.\mathsf{ctime} + \mathbb{A}.\mathsf{ctime}$. \\
We also note that the client can safely abort from the protocol by posting the refund transaction at any time before $T_0 - \mathbb{B}.\mathsf{ctime}$.
This construction allows to relax the time parameter $T_0$.
...
\begin{itemize}[topsep=0pt, itemsep=0pt, leftmargin=2em]
    ...
    \item $(\mathbf{tx}_{\mathbb{A}}, \mathbf{0/1})  \gets \mathbf{FullTx}_{(\mathbb{A})}(\mathsf{pk_{tx}}, \mathsf{pk_{rx}}, \mathsf{amnt}, \mathsf{sk_{tx}})$: an helper function that initializes a transaction, signs it with the given secret key and publishes it to the chain. Returns the transaction $\mathbf{tx}_{\mathbb{A}}$ and 1 if the transaction is accepted, 0 otherise.
\end{itemize}
...
\subsection{Using timelock puzzles}
...
\blockcomment{-0.278,-9.8}{-0.278,-0.5}{cblue}
\commentline{-0.65,-3.5}{Setup}
\blockcomment{-0.75,-12.4}{-0.75,-10.4}{cyellow}
\commentline{-1.2,-11.2}{Timeout}
\blockcomment{-1.2,-16.2}{-1.2,-12.6}{cred}
\commentline{-1.62,-15}{ForceSwap}
...
\subsection{Using commit transactions} 
...
If (2) occurs, note that when $P_1$ posts the refund transaction $\mathsf{tx_{rfnd(\mathbb{B})}}$ $P_0$ will obtain the signature $\sigma_{\mathsf{tx_{rfnd}}}$ that hashes to $\mathcal{H}(\sigma_{\mathsf{tx_{rfnd}}}) = \mathsf{hsig}$  and thus successfully execute the refund branch.

Since $P_1$ needs to post $\mathsf{tx_{rfnd(\mathbb{B})}}$ before $\mathbb{B}.\mathsf{ctime}$ and $T_1 = \mathbb{B}.\mathsf{ctime}$, the refund branch will complete before $2\mathbb{B}.\mathsf{ctime} + \mathbb{A}.\mathsf{ctime}$ after $P_1$ obtains $C_2$, so $P_0$ is guaranteed to publish and confirm $\mathsf{tx_{rfnd(\mathbb{A})}}$, resulting in $P_0$ holding $\mathsf{amnt_a}$ on $\mathsf{pk_{init(\mathbb{A})}}$.
