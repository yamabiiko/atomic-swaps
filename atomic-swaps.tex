...
\pseudocode{
    ...
    \mathsf{sk} := (r,s) := (r,\mathsf{msk}+\delta) \\
    \mathbf{return} \: (\mathsf{sk}, \mathsf{accd})
}
\vspace{1em}
\pseudocode{
    \mathsf{CmtTx}(\mathsf{st}, P, R, \mathcal{S}, \mathcal{T}) \\[0.1\baselineskip ][\hline]
    \{\mathsf{ssk}_i := (r_i, s_i),\mathsf{tsk}_i := (r'_i, s'_i), \mathsf{accd}_i\}_{i\in S} := \mathbf{parse} \: \mathcal{S} \\
    \{\mathsf{smpk}_i, \mathsf{tmpk}_i, \mathsf{accd}'_i\}_{i\in T} := \mathbf{parse} \: \mathcal{T} \\
    ...
    \;\: \mathsf{ac}'_i := (\mathsf{tpk}'_i, \mathsf{spk}'_i, \mathsf{co}'_i) \\
    \;\: \mathsf{tk}_i := (r'_i, \delta'_i, \delta''_i, \mathsf{accd}'_i) \\
...
\vspace{1em}
\pseudocode{
    \mathsf{CmtSrcChk}(\mathsf{ac}, \mathsf{sk}, \mathsf{accd}, \mathsf{clock}) \\[0.1\baselineskip ][\hline]
    (\mathsf{tpk}, \mathsf{spk}, \mathsf{co}) := \mathbf{parse} \: \mathsf{ac} \\
    \mathsf{time} := \mathbf{parse} \: \mathsf{accd} \\
    (r, s) := \mathbf{parse} \: \mathsf{sk} \\
    \mathbf{if} \: \mathsf{co} \neq \Gamma.\mathsf{Com}(\mathsf{accd}, r) \\
    \:\: \mathbf{return} \: 0 \\
    \mathbf{if} \: \mathsf{time} <= \mathsf{clock} \\
    \:\: \mathbf{return} \: \mathsf{tpk} = \Delta.\mathsf{KGen}(s) \\
    \mathsf{else} \\
    \:\: \mathbf{return} \: \mathsf{spk} = \Delta.\mathsf{KGen}(s) \\
}
\end{pcvstack}
\end{pchstack}
\end{minipage}%
